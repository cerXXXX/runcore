...
OUTROOT="$SCRIPT_DIR"
APPLE_FRAMEWORKS="$ROOT/flutter/native/apple/Frameworks"
...
header_src() {
  if [ -f "$APPLE_FRAMEWORKS/iOS/runcore.h" ]; then
    echo "$APPLE_FRAMEWORKS/iOS/runcore.h"
  elif [ -f "$APPLE_FRAMEWORKS/runcore.h" ]; then
    echo "$APPLE_FRAMEWORKS/runcore.h"
